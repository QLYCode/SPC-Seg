...
def _build_sam(
    ...
    if checkpoint is not None:
        ...
        # 只保留 image_encoder, prompt_encoder, pixel_mean, pixel_std 的参数
        allowed_prefixes = ["image_encoder", "prompt_encoder", "pixel_mean", "pixel_std"]
        filtered_state_dict = {
            k: v for k, v in state_dict.items()
            if any(k.startswith(p) for p in allowed_prefixes)
        }

        # 加载时设 strict=False，忽略掉 mask_decoder 等多余的部分
